...
int main(void) 
{
	buf = wbuf_init(BLOCKSZ);
	if (NULL != buf ) { 
		// at the begining fillin two buffer atonce		
		int addr = addr = buf->block;
		for (int i = 0; i < BLOCKSZ; i++, addr++) {
		...
		char done = 0;
		// start sampling in the timer interrupt handler
		ISR_timer();  		
		while (!done) { 
			wbuf_reload(&buf, data, DATA_LEN);
			// do anything else...
			// process(data...);
			continue;
		}
